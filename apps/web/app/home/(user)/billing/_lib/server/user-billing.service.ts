...
function getBaseUrl() {
  const override = process.env.NEXT_PUBLIC_BILLING_RETURN_BASE_URL;
  return appConfig.production ? (override ?? appConfig.url) : 'http://localhost:3000';
}

function getCheckoutSessionReturnUrl() {
  return new URL(pathsConfig.app.personalAccountBillingReturn, getBaseUrl()).toString();
}

function getBillingPortalReturnUrl() {
  return new URL(pathsConfig.app.personalAccountBilling, getBaseUrl()).toString();
}
