...
import { buildDomicilio, parseDate, parseTruthy, calcEstadoByFechaCita, normalizeFechaCitaISO, joinWithOtros, formatFechaNacimiento } from "./utils/format";
import { useCitasInfo } from "./hooks/useCitasInfo";
import { insertPacienteOffline, insertPacienteOnline } from "./services/pacientes";
...
export default function CrearPacientePage() {
  ...
  const { citasInfo, cargandoCitas } = useCitasInfo(isOnline, supabase, pacientesDB, userId);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (userError || !user || !isOnline) {
        const domicilioFinal = buildDomicilio(domicilioCompleto, domicilioFields, domicilio);
        const fc = startOfDay(fechaCita ? new Date(fechaCita) : new Date());
        const estadoCalc = calcEstadoByFechaCita(fc);
        const payload: PacienteInsertPayload = {
          nombre,
          edad: edad ? parseInt(edad) : null,
          fecha_nacimiento: formatFechaNacimiento(fechaNacimiento ?? null),
          sexo,
          domicilio: domicilioFinal,
          motivo_consulta: motivoConsulta === 'Otro' ? `Otro: ${motivoConsultaOtro}` : motivoConsulta,
          telefono,
          fecha_de_cita: normalizeFechaCitaISO(fechaCita),
          estado: estadoCalc,
          diagnostico_id: null,
          ocupacion,
          sintomas_visuales: joinWithOtros(
            sintomasVisualesSeleccionados,
            'Otro',
            sintomasVisualesOtro,
            sintomasVisuales
          ),
          antecedentes_visuales_familiares: joinWithOtros(
            antecedentesVisualesFamiliaresSeleccionados,
            'Otros',
            antecedentesVisualesFamiliaresOtros,
            antecedentesVisualesFamiliares
        ...
        await insertPacienteOffline(pacientesDB, payload);
      ...
      const domicilioFinal = buildDomicilio(domicilioCompleto, domicilioFields, domicilio);
      const estadoFinal = calcEstadoByFechaCita(fechaCita ? fechaCita : new Date());
      const payload: PacienteInsertPayload = {
        user_id: user.id,
        nombre,
        edad: edad ? parseInt(edad) : null,
      ...
      try {
        await insertPacienteOnline(supabase, payload);
  ...
  return (
    <>
      ...
      />
      <PageBody>
        <div className="container mx-auto w-full px-4 sm:px-6 py-6 pr-0 sm:pr-[120px]">
      <OfflineIndicators />
