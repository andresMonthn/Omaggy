'use client';
// coponente para la visualizacion dinamica de una sidebar inferior para dispositivos moviles
...
function isTeamRoute(pathname: string) {
  ...
  if (segs.length === 1) return false; // '/home'
  const second = segs[1] ?? '';

  // Ignore Next.js route groups like '(user)'
  if (second.startsWith('(') && second.endsWith(')')) return false;

  // Known personal routes under /home
  ...
  if (personalRoutes.includes(second)) return false;

  // Otherwise treat as team route: /home/<account-slug>/...
...
export default function MobileBottomBar() {
  ...
  useEffect(() => {
    ...
    const update = () => setShowMobile(mq.matches);
    update();
  ...
  const bar = (
    <nav className="fixed bottom-0 left-0 right-0 border-t border-gray-200 dark:border-primary/10 bg-background/80 backdrop-blur-md z-[1025]">
      <div className="w-full">
        <ul className={`grid ${!team ? 'grid-cols-8' : 'grid-cols-5'} items-center px-4 py-2 gap-x-2`}>
