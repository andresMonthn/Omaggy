/**
 * API de chat (assistant).
 * Función: Procesa prompts del usuario y delega en Ollama; maneja sesión/cookies.
 */
...
2) Para NAVEGAR a un módulo/ruta del sistema:
{
  ...
  "tipo": "ruta",
  "datos": {
    "destino": "dashboard|view|crearpaciente|agenda|inventario|qr|examenes|recetas|perfil|usuarios_admin",
...
export async function POST(req: Request) {
  try {
    ...
    let validated: { accion: string; tipo: string; datos: any } | null = null;
    try {
    ...
    const ollamaResp = await fetch(OLLAMA_URL, {
      ...
      if (ollamaResp.ok) {
        ...
        try {
          ...
          validated = validateAction(parsed);
        } catch {}
      }
    } catch {}

    if (!validated) {
      validated = inferActionFromPrompt(prompt);
    }
    ...
    if (accion === "crear" && tipo === "paciente") {
      ...
      const res = NextResponse.json({
        ...
        next: {
          action: "navigate",
          url: pathsConfig.app.crearpaciente,
          prefill: datosNorm,
        },
      });
      if (shouldResetCookie) {
    ...
    if (accion === "buscar" && tipo === "paciente") {
      ...
      if (list.length === 1) {
        const p = list[0];
        const url = `/home/historialclinico/${p.id}`;
        return NextResponse.json({
...
function inferActionFromPrompt(prompt: string): { accion: string; tipo: string; datos: any } | null {
  const text = String(prompt || "");
  const lower = text.toLowerCase();
  if (/(agenda|calendario)/.test(lower)) {
    return { accion: "navegar", tipo: "ruta", datos: { destino: "agenda" } };
  }
  if (/inventario/.test(lower)) {
    return { accion: "navegar", tipo: "ruta", datos: { destino: "inventario" } };
  }
  if (/(pacientes|dashboard|view)/.test(lower)) {
    return { accion: "navegar", tipo: "ruta", datos: { destino: "pacientes" } };
  }
  if (/(\bqr\b|c[oó]digo\s+qr)/.test(lower)) {
    return { accion: "navegar", tipo: "ruta", datos: { destino: "qr" } };
  }
  if (/buscar/.test(lower)) {
    const m = lower.match(/buscar\s+([^\n\.,]+)/);
    const nombre = m?.[1]?.trim() ?? "";
    return { accion: "buscar", tipo: "paciente", datos: { nombre } };
  }
  if (/(crear|registrar)/.test(lower)) {
    const datos = simpleExtractFromText(text);
    return { accion: "crear", tipo: "paciente", datos };
  }
  return null;
}

function simpleExtractFromText(original: string): any {
  const s = String(original || "");
  const lower = s.toLowerCase();
  const out: any = {};
  const edadMatch = lower.match(/(\d{1,3})\s*(años|anios|anos)/);
  if (edadMatch) out.edad = edadMatch[1];
  const telMatch = s.match(/\+?\d[\d\s\-]{6,}/g);
  if (telMatch && telMatch.length) {
    const digits = telMatch.map((t) => t.replace(/\D/g, "")).sort((a, b) => b.length - a.length)[0];
    out.telefono = digits;
  }
  if (/\bhoy\b/.test(lower)) out.fecha_de_cita = "hoy";
  const iso = lower.match(/\b(\d{4})-(\d{2})-(\d{2})\b/);
  const dmy = lower.match(/\b(\d{1,2})[\/\-](\d{1,2})[\/\-](\d{2,4})\b/);
  if (iso) out.fecha_de_cita = `${iso[1]}-${iso[2]}-${iso[3]}`;
  else if (dmy) {
    const dSrc = dmy?.[1] ?? "";
    const mSrc = dmy?.[2] ?? "";
    const ySrc = dmy?.[3] ?? "";
    const d = dSrc.padStart(2, "0");
    const m2 = mSrc.padStart(2, "0");
    const y = ySrc.length === 2 ? `20${ySrc}` : ySrc;
    out.fecha_de_cita = `${y}-${m2}-${d}`;
  }
  const fnISO = lower.match(/\b(\d{4})-(\d{2})-(\d{2})\b/);
  const fnDMY = lower.match(/\b(\d{1,2})[\/\-](\d{1,2})[\/\-](\d{2,4})\b/);
  if (fnISO) out.fecha_nacimiento = `${fnISO[1]}-${fnISO[2]}-${fnISO[3]}`;
  else if (fnDMY) {
    const dSrc = fnDMY?.[1] ?? "";
    const mSrc = fnDMY?.[2] ?? "";
    const ySrc = fnDMY?.[3] ?? "";
    ...
    const m2 = mSrc.padStart(2, "0");
    const y = ySrc.length === 2 ? `20${ySrc}` : ySrc;
    out.fecha_nacimiento = `${y}-${m2}-${d}`;
  }
  if (/(masculino|hombre)/.test(lower)) out.sexo = "masculino";
  if (/(femenino|mujer)/.test(lower)) out.sexo = "femenino";
  if (/lentes/.test(lower)) out.uso_lentes = true;
  const tipos: string[] = [];
  if (/armaz[oó]n/.test(lower)) tipos.push("armazón");
  if (/contacto|lentes de contacto/.test(lower)) tipos.push("contacto");
  if (/sol/.test(lower)) tipos.push("sol");
  if (/progresiv/.test(lower)) tipos.push("progresivos");
  if (tipos.length) out.tipos_de_lentes = tipos.join(", ");
  if (/cirug[ií]as?\s*(si|sí)/.test(lower) || /operaci[oó]n\s+ocular/.test(lower)) out.cirujias = true;
  else if (/cirug[ií]as?\s*no/.test(lower)) out.cirujias = false;
  if (/traumatismos?\s*(si|sí)/.test(lower)) out.traumatismos_oculares = true;
  else if (/traumatismos?\s*no/.test(lower)) out.traumatismos_oculares = false;
  return out;
}
