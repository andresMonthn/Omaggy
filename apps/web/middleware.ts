    ...
    },
    {
      ...
      handler: async (req: NextRequest, res: NextResponse) => {
        ...
        if (segments.length >= 3 && segments[0] === 'home') {
          ...
          const userId = (data.claims as { sub?: string })?.sub ?? null;
        ...
        const isBillingPage = (() => {
          ...
          const team = segments.length >= 3 && segments[0] === 'home'
            ? [
                pathsConfig.app.accountBilling.replace('[account]', segments[1]!),
                pathsConfig.app.accountBillingReturn.replace(
                  '[account]',
                  segments[1]!,
                ),
              ]
          ...
          return [...personal, ...team].some((p) => req.nextUrl.pathname.startsWith(p));
        })();
        ...
        if (typeof accountId === 'string' && accountId.length > 0) {
          ...
          if (!hasActive) {
            let redirectPath = pathsConfig.app.personalAccountBilling;

            if (segments.length >= 3 && segments[0] === 'home') {
              redirectPath = pathsConfig.app.accountBilling.replace(
                '[account]',
                segments[1]!,
              );
            }
